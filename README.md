...
Пайплайн автоматически распознаёт сноски в PDF и оформляет их в EPUB3 как интерактивные ссылки.

**Как это работает:**

1. **Детекция тела сноски** — блоки в нижних 35% страницы с уменьшенным шрифтом (<92% медианы), начинающиеся с цифры, определяются как тела сносок.
2. **Детекция маркера в тексте** — надстрочные цифры (superscript-флаг PyMuPDF) или цифры с уменьшенным шрифтом (<85% медианы блока) рядом с обычным текстом.
3. **Сквозная нумерация** — маркеры перенумеровываются подряд от 1 до N по всему документу, независимо от постраничной нумерации в оригинале.
4. **EPUB3-разметка** — в тексте: `<sup><a epub:type="noteref">`, в конце раздела: `<aside epub:type="footnote">`. Читалки показывают сноски во всплывающем окне.

Сноски работают автоматически, дополнительных флагов не требуется. Данные сохраняются в `footnotes.json` для использования из любого текстового источника.
